...
ext/zlib/exts.mk: FORCE
	$(Q)$(MINIRUBY) $(srcdir)/ext/extmk.rb --make='$(MAKE)' \
		--command-output=$@ $(EXTMK_ARGS) --extstatic $(EXTSTATIC) \
		-- configure $(@D)
gems: .bundle/gems/debug-1.9.2/exts.mk
.bundle/gems/debug-1.9.2/exts.mk: FORCE
	$(Q)$(MINIRUBY) $(srcdir)/ext/extmk.rb --make='$(MAKE)' \
		--command-output=$@ $(EXTMK_ARGS) --no-extstatic \
		-- configure $(@D)
gems: .bundle/gems/racc-1.7.3/exts.mk
.bundle/gems/racc-1.7.3/exts.mk: FORCE
	$(Q)$(MINIRUBY) $(srcdir)/ext/extmk.rb --make='$(MAKE)' \
		--command-output=$@ $(EXTMK_ARGS) --no-extstatic \
		-- configure $(@D)
gems: .bundle/gems/rbs-3.4.0/exts.mk
.bundle/gems/rbs-3.4.0/exts.mk: FORCE
	$(Q)$(MINIRUBY) $(srcdir)/ext/extmk.rb --make='$(MAKE)' \
		--command-output=$@ $(EXTMK_ARGS) --no-extstatic \
		-- configure $(@D)
